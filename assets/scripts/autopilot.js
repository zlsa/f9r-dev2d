...
var Autopilot = function(craft) {
  ...
  this.run = function() {
    ...
    var suicide_alt = this.get_suicide_alt(this.craft.getAltitude() - 8 + this.target_altitude, this.craft.getVspeed(), crange(0, 0.5, 1, 0.7, 1) * twr);
    ...
    if(this.started && !this.landed) {
      ...
      this.craft.throttle = crange(-10, this.pid.get(), 10, 0.015, 1);
//      this.craft.throttle = clamp(0.7, this.craft.throttle, 1);

      if(hit_time < 3 && !this.down) {
        this.craft.lowerGear();
        this.down = true;
      }
    }

    if(this.craft.getVspeed() > 0.1) this.landed = true;
